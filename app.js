document.addEventListener("DOMContentLoaded", () => {
        ...
        if (userData.equipment.length === 0) {
            ...
            userData.equipment.forEach((equipItem, idx) => {
                ...
                if (tbody) {
                    ...
                    const tdName = createCell(name);
                    
                    const tdCal = createCell(calibRaw); // Safely writes unicode explicitly
    ...
    function getEpsilonCoeffsForDetector(userData, currentDetName) {
        ...
        const matched = getIsotopeData(calib);
        let calEnergy = 0;
        ...
        userData.sources.forEach(src => {
            ...
            const mIso = getIsotopeData(src.isotope);
    ...
    function calculateCurrentActivity(src) {
        ...
        const matchedIso = getIsotopeData(src.isotope);
            ...
            if (hasSources) {
                ...
                userData.sources.forEach((src, idx) => {
                    ...
                    if (tbody) {
                        ...
                        const tdIso = createCell(isoText);
                        const tdA0 = createCell(formatActivity(src.activity));
                        const tdDate = createCell(dateFixed);
                        const tdAct = createCell(formatActivity(curAct));
    ...
    function renderTheoreticalTransmissionTable() {
        ...
        for (let i = 0; i < uniqueIsotopes.length; i += MAX_COLS) {
            ...
            displayAttenuatorsData.forEach((attObj, aIdx) => {
                ...
                const tdAtt = createCell(attObj.str);
                tr.appendChild(tdAtt);
        ...
        if (userData.attenuators.length === 0) {
            ...
            userData.attenuators.forEach((att, idx) => {
                ...
                if (checkboxTbody) {
                    ...
                    const tdMat = createCell(att.material);
                    
                    const tdThick = createCell(att.thickness); 
    ...
    function renderMeasurementsTable() {
        ...
        if (measArray.length === 0) {
            ...
            measArray.forEach(m => {
                ...
                const tdSrc = createCell(m.isoText);
                
                const tdAct = createCell(m.actStr);
                ...
                const tdDist = createCell(m.dist.toString());
                
                const tdRead = createCell(m.netReading);
    ...
    // Quick-Dismiss Tooltips via Interaction
    document.addEventListener("click", (e) => {
        if (document.body.classList.contains("show-tooltips")) {
            if (e.target.closest(".toggle-tooltips-btn")) return;
